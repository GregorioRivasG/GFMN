...
import { FiMapPin, FiZap, FiDroplet, FiThermometer, FiEdit2, FiTrash2, FiSave, FiXCircle, FiPlusCircle } from 'react-icons/fi';
import '../styles/ListaPecerasPage.css';
const ListaPecerasPage = () => {
    const [peceras, setPeceras] = useState([]);
    ...
    const fetchPeceras = () => {
        fetch(' http://45.174.90.242:5000/api/tanks')
            .then(res => {
            if (!res.ok)
                throw new Error('Error al obtener peceras');
            return res.json();
        })
    ...
    const eliminarPecera = async (id) => {
        ...
        if (!result.isConfirmed)
            return;
        try {
            const res = await fetch(` http://45.174.90.242:5000/api/tanks/${id}`, { method: 'DELETE' });
            if (!res.ok) {
    ...
    const abrirFormularioEdicion = (pecera) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editarId ? ` http://45.174.90.242:5000/api/tanks/${editarId}` : ' http://45.174.90.242:5000/api/tanks';
            ...
            if (!response.ok)
                throw new Error(data.error || 'Error al guardar pecera');
            await Swal.fire({
                position: 'top',
                title: 'Éxito',
    ...
    return (<div className="lista-peceras-container">
      <Navbar />
      ...
      {peceras.length === 0 ? (<p>No hay peceras registradas.</p>) : (<table className="tabla-peceras">
          <thead>
            <tr>
              <th>#</th>
              <th>Ubicación</th>
          ...
          <tbody>
            {peceras.map((p, i) => (<tr key={p._id}>
                ...
                <td>
                  <button className="btn-editar" onClick={() => abrirFormularioEdicion(p)} title="Editar pecera" aria-label="Editar pecera">
                    <FiEdit2 size={18}/>
                  </button>{' '}
                  <button className="btn-eliminar" onClick={() => eliminarPecera(p._id)} title="Eliminar pecera" aria-label="Eliminar pecera">
                    <FiTrash2 size={18}/>
                  </button>
                </td>
              </tr>))}
      ...
      {mostrarForm && (<div className="modal-fondo">
          <div className="form-popup">
            <h3>{editarId ? 'Editar Pecera' : 'Nueva Pecera'}</h3>

            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              <FiMapPin size={20}/>
              <input type="text" name="ubicacion" placeholder="Ubicación" value={formData.ubicacion} onChange={handleChange} autoFocus/>
            </label>

            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              <FiZap size={20}/>
              <input type="text" name="conductividad" placeholder="Conductividad" value={formData.conductividad} onChange={handleChange}/>
            </label>

            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              <FiDroplet size={20}/>
              <input type="text" name="ph" placeholder="Nivel de pH" value={formData.ph} onChange={handleChange}/>
            </label>

            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              <FiThermometer size={20}/>
              <input type="text" name="temperatura" placeholder="Temperatura (°C)" value={formData.temperatura} onChange={handleChange}/>
            </label>
