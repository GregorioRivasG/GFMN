...
const AlertasPage = () => {
    ...
    useEffect(() => {
        ...
        const alertasIniciales = guardadas ? JSON.parse(guardadas) : [];
        // Obtener última medición de la API
        const fetchLatestMeasurement = async () => {
            try {
                const token = localStorage.getItem('token');
                const response = await fetch(' http://45.174.90.242:5000/api/measurements/latest', {
                    headers: {
    ...
    const handleAcknowledge = (id) => {
        const updatedAlertas = alertas.map(alerta => alerta.id === id ? { ...alerta, admitido: true } : alerta);
        setAlertas(updatedAlertas);
        localStorage.setItem('alertas-tabla', JSON.stringify(updatedAlertas));
    };
    return (<div className="alertas-container">
      <Navbar />
      <div className="alertas-tabla-contenido">
        ...
        <table className="tabla-alertas">
          ...
          <tbody>
            {alertas.map((a) => (<tr key={a.id} className={a.admitido ? 'alerta-admitida' : ''}>
                <td>
                ...
                <td>
                  {!a.admitido && (<button onClick={() => handleAcknowledge(a.id)} className="boton-admitir">
                      Admitir
                    </button>)}
                  {a.admitido && '✅ Admitido'}
                </td>
