...
const InformesPage = () => {
    ...
    const informesFiltrados = informes
        .filter((inf) => `${inf.titulo} ${inf.descripcion} ${inf.fechaInicio} ${inf.fechaFin} ${inf.peceraNombre}`
        .toLowerCase()
    ...
    return (<div className="informes-container">
      <Navbar />
      <div className="informes-header">
        <h2>Informes históricos</h2>
        <div className="barra-superior">
          <input type="text" placeholder="🔍 Buscar" value={busqueda} onChange={(e) => {
      ...
      <div className="lista-informes">
        {paginados.map((inf) => (<div key={inf.id} className="item-informe">
            ...
            <div className="acciones-informe">
              {inf.fileUrl && (<a href={inf.fileUrl} download className="btn-descargar" title="Descargar PDF">
                  <FiDownload size={18}/>
                </a>)}
              <button onClick={() => eliminarInforme(inf.id)} className="btn-eliminar" title="Eliminar informe">
                <FiTrash2 size={18}/>
              </button>
      ...
      {/* Paginación */}
      {totalPaginas > 1 && (<div className="paginacion">
