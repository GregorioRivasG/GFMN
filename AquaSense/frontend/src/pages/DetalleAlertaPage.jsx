...
const DetalleAlertaPage = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const [alerta, setAlerta] = useState(null);
    useEffect(() => {
        const data = JSON.parse(localStorage.getItem('alertas-detalle') || '[]');
        const encontrada = data.find((a) => a.id === id);
        if (encontrada)
            setAlerta(encontrada);
    }, [id]);
    const actualizar = (campo) => {
        const data = JSON.parse(localStorage.getItem('alertas-detalle') || '[]');
    ...
    if (!alerta)
        return <div className="alerta-detalle-container"><Navbar /><p style={{ padding: 20 }}>No se encontró la alerta.</p></div>;
    return (<div className="alerta-detalle-container">
      <Navbar />
