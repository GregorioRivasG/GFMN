...
ChartJS.register(LineElement, CategoryScale, LinearScale, PointElement, Tooltip, Legend, Colors);
const DetallePeceraPage = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const tankResponse = await fetch(` http://45.174.90.242:5000/api/tanks/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
                if (!tankResponse.ok)
                    throw new Error('Error al obtener pecera');
                const tankData = await tankResponse.json();
                setTank(tankData);
                const measurementsResponse = await fetch(` http://45.174.90.242:5000/api/measurements?tankId=${id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    const filterMeasurementsByDate = (fechaInicio, fechaFin) => {
    ...
    const generatePDF = async (fechaInicio, fechaFin) => {
        if (!reportRef.current || !tank)
            return;
        filterMeasurementsByDate(fechaInicio, fechaFin);
        try {
    ...
    if (loading)
        return <div className="detalle-container"><Navbar /><p style={{ padding: 20 }}>Cargando...</p></div>;
    if (error)
        return <div className="detalle-container"><Navbar /><p style={{ padding: 20 }}>Error: {error}</p></div>;
    if (!tank)
        return <div className="detalle-container"><Navbar /><p style={{ padding: 20 }}>Pecera no encontrada</p></div>;
    ...
    return (<div className="detalle-container">
      ...
      <div className="detalle-content">
        ...
        <div className="report-header">
          ...
          <button onClick={() => setShowReportForm(true)} className="btn-generar-reporte">
          ...
          <div className="measurements-table">
            ...
            <div className="table-container">
              <table>
                ...
                <tbody>
                  {filteredMeasurements.slice().reverse().map((m) => (<tr key={m._id}>
        ...
        {showReportForm && (<div className="modal-fondo">
            <GenerarReporteForm onGenerate={generatePDF} onCancel={() => setShowReportForm(false)}/>
          </div>)}
      </div>
    </div>);
};
