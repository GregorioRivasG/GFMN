...
const InformeDetallePage: React.FC = () => {
  ...
  const descargarPDF = () => {
    ...
    const opt = {
      margin: 10,
      filename: `${informe?.titulo || 'reporte'}.pdf`,
  ...
  if (!informe) return <div className="detalle-informe-container"><Navbar /><p>Cargando informe...</p></div>;
      ...
      {modoEdicion && (
        <div className="modal-fondo">
          <div className="form-popup">
            ...
            />
            <input 
              value={editado?.fechaInicio || ''} 
              type="date" 
              onChange={e => setEditado({ ...editado, fechaInicio: e.target.value })} 
            />
            <input 
              value={editado?.fechaFin || ''} 
              type="date" 
              onChange={e => setEditado({ ...editado, fechaFin: e.target.value })} 
            />
            <textarea 
            ...
            />
            <input 
              value={editado?.oxigeno || ''} 
              onChange={e => setEditado({ ...editado, oxigeno: e.target.value })} 
              placeholder="Oxígeno disuelto"
            />
            <input 
              value={editado?.ph || ''} 
              onChange={e => setEditado({ ...editado, ph: e.target.value })} 
              placeholder="Nivel de pH"
            />
            <input 
              value={editado?.temperatura || ''} 
              onChange={e => setEditado({ ...editado, temperatura: e.target.value })} 
